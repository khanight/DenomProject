...
| `/keep [guidance]` | Commits chosen parts of your **most recent `/brainstorm` reply** (for the active project) into its files — nothing from a brainstorm is ever applied automatically; this is the deliberate opt-in step. Reference the brainstorm's numbers directly: `/keep 1, 3, 5; amend 2: <replacement text>; scrap 4`. With no guidance, Claude uses judgment — committing concrete, decided ideas and leaving out anything that read as one option among several you hadn't chosen. Same synthesize-and-merge behavior as `/braindump`. Replies with what it kept and what it deliberately left out. If you haven't run `/brainstorm` yet for this project (or the bot restarted since), it tells you so instead of guessing. |
